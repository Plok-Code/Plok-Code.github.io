(() => {
  ...
  const initCardDecks = () => {
    ...
    for (const deck of decks) {
      if (deck.dataset.deckInit === "1") continue;
      deck.dataset.deckInit = "1";

      const cards = Array.from(deck.querySelectorAll("[data-deck-card]"));
      const article = deck.querySelector("[data-deck-article]");
      const title = deck.querySelector("[data-deck-article-title]");
      const content = deck.querySelector("[data-deck-article-content]");

      if (!cards.length || !article || !title || !content) continue;
      ...
      const setActive = (card) => {
        ...
        for (const item of cards) {
          ...
          item.classList.toggle("is-flipped", isSelected);
        ...
        const cardTitle = card.querySelector(".deck-card__title")?.textContent?.trim() || "D\u00e9tail";
        const key = card.dataset.cardKey;
        const template = key ? deck.querySelector(`template[data-card-template="${key}"]`) : null;

        title.textContent = cardTitle;
        content.replaceChildren();
        if (template?.content) content.appendChild(template.content.cloneNode(true));
